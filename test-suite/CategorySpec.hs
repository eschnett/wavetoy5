...
prop_FCompose_compose (Fn f) (Fn g) x =
    chase (FCompose f g) x === (f . g) x

prop_FCompose_left_id :: Fun A B -> A -> Property
prop_FCompose_left_id (Fn f) x =
    chase (FIdentity `FCompose` f) x === chase f x

prop_FCompose_right_id :: Fun A B -> A -> Property
prop_FCompose_right_id (Fn f) x =
    chase (f `FCompose` FIdentity) x === chase f x
